...
func TestP2PKH(t *testing.T) {
	...
	require.NoError(t, err, "unexpected error")
	s, err := Disassemble(P2PKHPublicBytes(key.PubKey().SerializeCompressed()))
	require.NoError(t, err, "unexpected error")
...
func TestP2PK(t *testing.T) {
	key, err := btcec.NewPrivateKey(btcec.S256())
	require.NoError(t, err, "unexpected error")
	s, err := Disassemble(P2PKPublicBytes(key.PubKey().SerializeCompressed()))
	require.NoError(t, err, "unexpected error")
...
func TestP2MS(t *testing.T) {
	...
	s, err := Disassemble(P2MS(false, RKEYS, keys...))
	require.NoError(t, err, "unexpected error")
...
func TestP2SH(t *testing.T) {
	sc := Int64(1234)
	s, err := Disassemble(P2SHScript(sc))
	require.NoError(t, err, "unexpected error")
...
func TestLockTime(t *testing.T) {
	n := time.Now()
	s, err := Disassemble(LockTimeTime(n))
	require.NoError(t, err, "unexpected error")
	b := make([]byte, 4)
